...
var require_playwright_grpc_pb = __commonJS({
  "node/playwright-wrapper/generated/playwright_grpc_pb.js"(exports2) {
    ...
    function serialize_Request_CoverageStart(arg) {
      if (!(arg instanceof playwright_pb.Request.CoverageStart)) {
        throw new Error("Expected argument of type Request.CoverageStart");
      }
      return Buffer.from(arg.serializeBinary());
    }
...
async function getSelections(locator) {
  ...
  if (selectOptions) {
    ...
    logger7.info(`Option entries: ${entries.length}`);
    logger7.info(`Selected entries: ${entries.filter((e) => e.getSelected()).length}`);
...
async function getText(request2, state2) {
  ...
  try {
    ...
    logger7.info(`Retrieved text for element ${selector} containing ${content}`);
  } catch (e) {
...
async function getProperty(request2, state2) {
  ...
  try {
    ...
    logger7.info(`Retrieved dom property for element ${selector} containing ${content}`);
    return content;
  } catch (e) {
    if (e instanceof Error) {
      logger7.error(e);
...
async function getAttributeValue(request2, state2) {
  ...
  logger7.info(`Retrieved attribute for element ${selector} containing ${attribute}`);
  return attribute;
}
var stateEnum = {
  attached: 1,
  ...
  try {
    ...
    try {
      ...
      if (editable && disabled === null) {
        logger7.info(`Element ${selector} is editable: ${editable}`);
        states += stateEnum.editable;
      } else if (!editable && disabled !== null) {
        logger7.info(`Element ${selector} is disabled: ${disabled}`);
        states += stateEnum.readonly;
      } else {
        logger7.info(`Element ${selector} is readonly: ${!editable}`);
        states += stateEnum.readonly;
      }
    } catch (error) {
      logger7.info(`Element ${selector} is not editable: ${error}`);
      if (disabled !== null) {
        logger7.info(`Element ${selector} is disabled: ${disabled} and therefore readonly`);
        states += stateEnum.readonly;
      }
    }
    logger7.info("Checking checked state");
...
async function getStyle(request2, state2) {
  ...
  const strictMode = request2.getStrict();
  logger7.info("Getting css of element on page");
  const locator = await findLocator(state2, selector, strictMode, void 0, true);
...
var import_pino8 = __toESM(require("pino"));
var logger8 = (0, import_pino8.default)({ timestamp: import_pino8.default.stdTimeFunctions.isoTime });
async function selectOption(request2, state2) {
  ...
  if (result.length == 0) {
    logger8.info("Couldn't select any options");
...
async function internalClick(selector, strictMode, options, state2) {
  ...
  try {
    ...
    if (error instanceof Error && error.message.startsWith("locator.click: Target page, context or browser has been closed")) {
      logger8.warn("Supress locator.click: Target page, context or browser has been closed error");
      return false;
    }
    throw error;
  }
...
async function uploadFileBySelector(request2, state2) {
  ...
  if (path4.length === 0) {
    ...
    logger8.info(`Uploading file ${name} as buffer to ${selector}`);
    ...
    logger8.info(`Uploading file(s) ${path4} to ${selector}`);
...
async function handleAlert(request2, page) {
  ...
  const fn = async (dialog) => {
    ...
    logger8.info(`Alert text: ${dialogueText}`);
  };
  page.on("dialog", fn);
...
async function waitForAlerts(request2, page) {
  ...
  for (let index = 0; index < alertActions.length; index++) {
    ...
    logger8.info(`Waiting for alert with action: ${action}, promptInput: "${promptInput}" and timeout: ${timeout}`);
...
var extractArgumentsStringFromJavascript = (javascript) => {
  ...
  logger9.error(`Could not extract arguments from javascript:
${javascript}
defaulting to *args`);
  return "*args";
};
async function initializeExtension(request, state) {
  logger9.info(`Initializing extension: ${request.getPath()}`);
  ...
  logger9.info(`Adding ${kws.length} keywords from JS Extension`);
...
async function _connectBrowser(browserName, url, connectCDP, timeout) {
  logger9.info(`Connecting to browser: ${browserName} at ${url} via ${connectCDP ? "CDP" : "WebSocket"}`);
...
async function _createIndexedContext(context, defaultTimeout, traceFile, options) {
  ...
  if (traceFile) {
    logger9.info("Tracing enabled with: { screenshots: true, snapshots: true }");
...
async function _newPage(context, page = void 0) {
  ...
  newPage2.on("close", () => {
    ...
    if (oldPageStackLength != filteredPageStack.length) {
      ...
      logger9.info("Removed " + contextPage.id + " from " + context.id + " page stack");
    }
  });
  return contextPage;
}
var PlaywrightState = class {
  ...
  addBrowser(browserAndConfs) {
    ...
    logger9.info(`Adding browser to stack: ${browserAndConfs.browserType}, version: ${adding_browser?.version()}`);
    ...
    if (browserContexts !== void 0) {
      logger9.info(`Adding ${browserContexts.length} contexts to browser`);
      for (const context of browserContexts) {
        const indexedPages = context.pages().map((page) => indexedPage(page));
        logger9.info(`Adding ${indexedPages.length} pages to context`);
        const indexedContext = {
...
var BrowserState = class {
  ...
  pushPage(newPage2) {
    ...
    if (newPage2 !== void 0 && currentContext !== void 0) {
      ...
      logger9.info("Changed active page");
    } else {
      logger9.info("Set active page to undefined");
    }
  }
  get contextStack() {
...
async function closePage(request2, openBrowsers) {
  ...
  logger9.info(`Closing page with runBeforeUnload ${unload}`);
...
async function newPage(request2, openBrowsers) {
  ...
  try {
    ...
    logger9.info("Suppress video().path() error");
  }
  logger9.info("Video path: " + videoPath);
...
async function newContext(request2, openBrowsers) {
  ...
  logger9.info("Creating new context with options: " + JSON.stringify(options));
  ...
  logger9.info(`Trace file: ${traceFile}`);
...
async function launchBrowserServer(request2, openBrowsers) {
  const browserType = request2.getBrowser();
  const options = JSON.parse(request2.getRawoptions());
  logger9.info(`Launching browser server: ${browserType}`);
  logger9.info("Launching browser server with options: " + JSON.stringify(options));
  ...
  logger9.info(`Browser server launched. Endpoint: ${browserServer.wsEndpoint()}`);
...
async function _switchPage(id, browserState) {
  ...
  logger9.info("Changing current active page: " + id);
...
async function startCoverage(request2, state2) {
  ...
  if (["css", "all"].includes(coverageOptions.type)) {
    logger9.info(`Starting CSS coverage with resetOnNavigation: ${resetOnNavigation}`);
...
async function _saveCoverageReport(activeIndexedPage) {
  ...
  if (raw && configFile === "") {
    logger9.info("Raw and v8 coverage enabled");
    options.reports = [["raw"], ["v8"]];
  } else {
    logger9.info("v8 coverage disabled");
  }
  const mcr = new import_monocart_coverage_reports.CoverageReport(options);
  if (configFile) {
    logger9.info({ "Config file: ": configFile });
...
var import_pino10 = __toESM(require("pino"));
var logger10 = (0, import_pino10.default)({ timestamp: import_pino10.default.stdTimeFunctions.isoTime });
async function addLocatorHandlerCustom(request2, state2) {
  ...
  logger10.info(`Adding locator handler for ${overlaySelector} as times: ${times}`);
  ...
  await activePage.addLocatorHandler(
    overlayLocator,
    async () => {
      logger10.info(`Overlay locator ${overlaySelector} is found`);
      for (const handlerSpec of hadlerSpecs) {
        ...
        try {
          ...
          logger10.error(`Error in custom locator handler: ${error}`);
        }
      }
    },
    { times, noWaitAfter }
...
async function removeLocatorHandler(request2, state2) {
  logger10.info(`Removing locator handler for ${request2.getSelector()}`);
  const activePage = state2.getActivePage();
  exists(activePage, "Could not find active page");
...
async function savePageAsPdf(request2, state2) {
  ...
  const width = request2.getWidth();
  logger11.info(`Saving pdf to ${pdfPath}`);
...
async function emulateMedia(request2, state2) {
  ...
  if (colorScheme === "not_set") {
    logger11.info(`Emulating colorScheme not set`);
  } else if (colorScheme === "null") {
    logger11.info(`Emulating colorScheme null`);
    options.colorScheme = null;
  } else {
    logger11.info(`Emulating colorScheme ${colorScheme}`);
    options.colorScheme = colorScheme;
  }
  const forcedColors = request2.getForcedcolors();
  if (forcedColors === "not_set") {
    logger11.info(`Emulating forcedColors not set`);
  } else if (forcedColors === "null") {
    logger11.info(`Emulating forcedColors null`);
    options.forcedColors = null;
  } else {
    logger11.info(`Emulating forcedColors ${forcedColors}`);
    options.forcedColors = forcedColors;
  }
  const media = request2.getMedia();
  if (media === "not_set") {
    logger11.info(`Emulating media not set`);
  } else if (media === "null") {
    logger11.info(`Emulating media null`);
    options.media = null;
  } else {
    logger11.info(`Emulating media ${media}`);
    options.media = media;
  }
  const reducedMotion = request2.getReducedmotion();
  if (reducedMotion === "not_set") {
    logger11.info(`Emulating reducedMotion not set`);
  } else if (reducedMotion === "null") {
    logger11.info(`Emulating reducedMotion null`);
    options.reducedMotion = null;
  } else {
    logger11.info(`Emulating reducedMotion ${reducedMotion}`);
...
function async_logger(propertyKey, propertyDescriptor) {
  ...
  propertyDescriptor.value = async function(...args2) {
    try {
      logger12.info(`Start of node method ${propertyKey}`);
      const result = await originalMethod.apply(this, args2);
      logger12.info(`End of node method ${propertyKey}`);
      return result;
    } catch (err) {
      logger12.info(`Error of node method  ${propertyKey}`);
      throw err;
    }
  };
...
var PlaywrightServer = class {
    ...
    this.wrapping = (func) => {
      return async (call, callback) => {
        try {
          ...
          logger13.info(`Start of node method ${func.name}`);
          const response = await func(request2, this.getState(call));
          logger13.info(`End of node method ${func.name}`);
          callback(null, response);
        } catch (e) {
          logger13.info(`Error of node method  ${func.name}`);
  ...
  async waitForResponse(call) {
    try {
      ...
      for (const result of results) {
        logger13.info(`Sending response ${result.getLog()}`);
        call.write(result);
      }
    } catch (e) {
      call.emit("error", errorResponse(e));
  ...
  async uploadFileBySelector(call, callback) {
    ...
    call.on("data", async (request2) => {
      try {
        logger13.info(`Reading multiplepart uploadFileBySelector`);
...
server.bindAsync(`${host}:${port}`, import_grpc_js2.ServerCredentials.createInsecure(), () => {
  logger14.info(`Listening on ${host}:${port}`);
  server.start();
});
